...
public class FieldPolygon {
  ...
  public int checkRobotPosition(Pose2d pose) {
    ...
    if (pointInPolygon(x, y, getYellowPoints())) return 1;
    if (pointInPolygon(x, y, getGreenPoints()))  return 2;
    if (pointInPolygon(x, y, getBluePoints()))   return 3;
    if (pointInPolygon(x, y, getPinkPoints()))   return 4;
    if (pointInPolygon(x, y, getRedPoints()))    return 5;
    if (pointInPolygon(x, y, getOrangePoints())) return 6;
    return 0;
  }
